...
const NAV_BY_ROLE = {
  admin:      ["dashboard","students","results","attendance","billing","discipline","registration","reports","assets","notices","users"],
  principal:  ["dashboard","students","results","attendance","discipline","reports","notices"],
  teacher:    ["dashboard","students","results","attendance","discipline","notices"],
  accountant: ["dashboard","billing","students","reports","assets"],
...
function Students({user}) {
  ...
  const {data,loading,error,reload}=useFetch(()=>api.students({search:dSearch,grade:form_filter,page:pg,limit:20}),[dSearch,form_filter,pg]);
  ...
  const handleSave=async()=>{
    ...
    try {
      ...
      if(modal==="add" && createParent && parentForm.username && parentForm.password && parentForm.fullName) {
        ...
        if(newStudentId && parentRes.user?.id) {
          await api.linkParentStudent({parentUserId:parentRes.user.id,studentId:newStudentId});
        }
        ...
        <Modal title={modal==="view"?"Learner Details":modal==="edit"?"Edit Learner":"Enrol New Learner"} onClose={()=>setModal(null)}>
          {modal==="view"&&selected?(
            <div>
              ...
              <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}>
                <Btn size="sm" icon="download" variant="tq"
                  onClick={()=>window.open(api.reportCardURL(selected.id,{term:"Term 1",year:CURRENT_YEAR,report_type:"term_report"}),"_blank")}>
                  Term Report PDF
                </Btn>
                <Btn size="sm" icon="download" variant="secondary"
                  onClick={()=>window.open(api.reportCardURL(selected.id,{term:"Term 1",year:CURRENT_YEAR,report_type:"mark_reader"}),"_blank")}>
                  Mark Reader PDF
                </Btn>
              </div>
            </div>
            ...
            <>
              ...
              {/* Parent Login Account — only when adding new student */}
              {modal==="add"&&(
                <div style={{marginTop:8,padding:16,borderRadius:10,border:`2px dashed ${createParent?theme.turquoise:theme.border}`,background:createParent?"#F0FDFF":"#FAFAFA",transition:"all 0.2s"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:createParent?14:0}}>
                    ...
                    <button type="button" onClick={()=>setCreateParent(v=>!v)}
                      style={{width:44,height:24,borderRadius:12,border:"none",cursor:"pointer",background:createParent?theme.turquoise:theme.border,transition:"all 0.2s",position:"relative"}}>
                  ...
                  {createParent&&(
                    <>
                      <div className="form-group" style={{marginBottom:10}}>
                        ...
                        <input className="form-input" placeholder="e.g. Blessing Moyo" value={parentForm.fullName} onChange={pf("fullName")}/>
                      </div>
                      <div className="form-row">
                        <div className="form-group" style={{marginBottom:10}}>
                          <label>Username *</label>
                          <input className="form-input" placeholder="e.g. bmoyo" value={parentForm.username} onChange={pf("username")}/>
                        </div>
                        <div className="form-group" style={{marginBottom:10}}>
                          <label>Password *</label>
                          <input type="password" className="form-input" placeholder="Min 6 characters" value={parentForm.password} onChange={pf("password")}/>
                        </div>
                      </div>
                      <div style={{padding:"8px 12px",background:"#CFFAFE",borderRadius:8,fontSize:11,color:"#0E7490"}}>
                        ✓ Account will be <strong>immediately active</strong> — parent can log in straight away.
                      </div>
                    </>
                  )}
                </div>
...
function Discipline({user}) {
  ...
  const incidents=[
    {id:"INC001",student:"Sibanda, Samuel",form:"Form 5",incident:"Mobile phone in class",date:"2026-01-15",severity:"Low",action:"Phone confiscated, $100 fine levied",status:"Open"},
    {id:"INC002",student:"Matongo, Kupakwashe",form:"Form 5",incident:"Tardiness — repeated",date:"2026-01-18",severity:"Medium",action:"Parent contacted, written warning",status:"Open"},
    {id:"INC003",student:"Mukonda, Chantaille",form:"Form 5",incident:"Uniform infringement",date:"2026-01-20",severity:"Low",action:"Verbal warning issued",status:"Resolved"},
    {id:"INC004",student:"Charima, Jacob",form:"Form 5",incident:"Disruption in class",date:"2026-01-22",severity:"Medium",action:"Detention — Friday",status:"Resolved"},
  ];
  const f=k=>e=>setForm(p=>({...p,[k]:e.target.value}));
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><h2 style={{fontSize:20,fontWeight:700,fontFamily:"'Playfair Display',serif"}}>Discipline Management</h2><p style={{fontSize:13,color:theme.textMuted}}>Merits, demerits & incidents · Full backend coming soon</p></div>
        {canEdit&&<Btn icon="plus" onClick={()=>setModal(true)}>Log Incident</Btn>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:20}}>
        {[["Open Cases",2,theme.danger],["Resolved",2,theme.green],["This Term",4,theme.amber]].map(([l,v,c])=>(
          <div key={l} style={{background:"white",padding:20,borderRadius:12,textAlign:"center",boxShadow:"0 2px 8px rgba(196,30,58,0.06)",border:`1px solid ${theme.border}`}}><p style={{fontSize:28,fontWeight:700,color:c,fontFamily:"'Playfair Display',serif"}}>{v}</p><p style={{fontSize:12,fontWeight:600,color:theme.textMuted,textTransform:"uppercase"}}>{l}</p></div>
        ))}
      </div>
      <div style={{background:"white",borderRadius:14,boxShadow:"0 2px 8px rgba(196,30,58,0.06)",border:`1px solid ${theme.border}`,overflow:"hidden"}}>
        <table><thead><tr style={{background:"#FEF2F2"}}><th>ID</th><th>Learner</th><th>Incident</th><th>Date</th><th>Severity</th><th>Action Taken</th><th>Status</th></tr></thead>
          <tbody>{incidents.map(d=>(
            <tr key={d.id}>
              <td style={{fontFamily:"monospace",fontSize:12,color:theme.turquoise,fontWeight:600}}>{d.id}</td>
              <td><p style={{fontWeight:500,fontSize:13}}>{d.student}</p><p style={{fontSize:11,color:theme.textMuted}}>{d.form}</p></td>
              <td style={{fontSize:13}}>{d.incident}</td>
              <td style={{fontSize:12,color:theme.textMuted}}>{d.date}</td>
              <td><Badge color={d.severity==="High"?"red":d.severity==="Medium"?"amber":"tq"}>{d.severity}</Badge></td>
      ...
      {modal&&<Modal title="Log Incident" onClose={()=>setModal(false)}>
        <div className="form-row"><div className="form-group"><label>Learner Name *</label><input className="form-input" value={form.student||""} onChange={f("student")}/></div><div className="form-group"><label>Form</label><select className="form-input" value={form.form||""} onChange={f("form")}><option value="">Select</option>{FORMS.map(fm=><option key={fm} value={fm}>{FORM_LABEL[fm]}</option>)}</select></div></div>
        <div className="form-group"><label>Incident Description *</label><textarea className="form-input" rows={3} value={form.incident||""} onChange={f("incident")}/></div>
        <div className="form-row"><div className="form-group"><label>Severity</label><select className="form-input" value={form.severity||"Low"} onChange={f("severity")}><option>Low</option><option>Medium</option><option>High</option></select></div><div className="form-group"><label>Date</label><input type="date" className="form-input" value={form.date||new Date().toISOString().split("T")[0]} onChange={f("date")}/></div></div>
        <div className="form-group"><label>Action Taken</label><input className="form-input" value={form.action||""} onChange={f("action")}/></div>
        <div style={{padding:12,background:"#FEF3C7",borderRadius:8,fontSize:12,color:"#92400E",marginBottom:16}}>⚠️ Full discipline backend connecting in next phase. Form captures for now.</div>
        <div style={{display:"flex",gap:10}}><Btn onClick={()=>setModal(false)}>Submit</Btn><Btn variant="secondary" onClick={()=>setModal(false)}>Cancel</Btn></div>
      </Modal>}
    </div>
  );
}
...
function UserManagement({user}) {
  ...
  const [modal,    setModal]    = useState(null);
  const [saving,   setSaving]   = useState(false);
  const [form,     setForm]     = useState({role:"teacher", campus:"swla"});
  const [search,   setSearch]   = useState("");
  ...
  const searchStudentsForLink = async () => {
    ...
    if (!q) { setLinkError("Please enter a name or student ID"); return; }
    setLinkLoading(true); setLinkError(""); setLinkSearchResults([]);
    try {
      const res = await api.get("/students?search=" + encodeURIComponent(q) + "&limit=10");
      if (!res.students || res.students.length === 0) {
        setLinkError(`No learners found for "${q}". Try the learner surname.`);
      } else {
        setLinkSearchResults(res.students);
      }
    } catch(e) {
  ...
  const displayUsers = (tab==="pending" ? pendingUsers : allUsers).filter(u=>
  ...
  const handleCreate = async () => {
    if (!form.fullName || !form.username || !form.password) {
      toast("Full name, username and password are required", "warning"); return;
    }
    if (form.password.length < 6) { toast("Password must be at least 6 characters", "warning"); return; }
    setSaving(true);
    try {
      const res = await api.createUser({
        full_name: form.fullName, username: form.username,
        email: form.email || "", password: form.password,
        role: form.role, campus: form.campus,
        is_approved: true,
      });
      if (form.role === "parent" && form.linkedStudentId && res.user?.id) {
        await api.linkParentStudent({parentUserId: res.user.id, studentId: form.linkedStudentId});
        toast(`Parent account created & linked to ${form.linkedStudentName || "learner"}`);
      } else {
        toast(`${ROLE_STYLE[form.role]?.label||form.role} account created — they can log in now`);
      }
      setModal(null);
      setForm({role:"teacher", campus:"swla"});
      setLinkStudentId(""); setLinkSearchResults([]);
      reloadAll();
    } catch(e) { toast(e.message, "error"); }
    finally { setSaving(false); }
  };
    ...
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div>
          ...
          <p style={{fontSize:13,color:theme.textMuted}}>Manage staff, teacher and parent accounts</p>
        </div>
        <div style={{display:"flex",gap:8}}>
          ...
          <Btn icon="plus" onClick={()=>{setForm({role:"teacher",campus:"swla"});setLinkStudentId("");setLinkSearchResults([]);setModal("create");}}>
            Create Account
          </Btn>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div style={{display:"flex",gap:6,marginBottom:16,borderBottom:`1px solid ${theme.border}`,paddingBottom:0}}>
        {[["all",`All Users (${allUsers.length})`],["pending",`Pending Approval (${pendingUsers.length})`]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{padding:"10px 18px",border:"none",background:"none",cursor:"pointer",fontWeight:tab===id?700:500,fontSize:13,color:tab===id?theme.red:theme.textMuted,borderBottom:`2px solid ${tab===id?theme.red:"transparent"}`,transition:"all 0.15s"}}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab==="pending" && pendingUsers.length>0 && (
        <div style={{background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:10,padding:14,marginBottom:16,display:"flex",gap:10,alignItems:"center"}}>
          ...
          <p style={{fontSize:13,color:"#92400E",fontWeight:600}}>
            {pendingUsers.length} account{pendingUsers.length>1?"s":""} waiting for your approval. Review and approve below.
          </p>
        </div>
      )}
      ...
      <div style={{background:"white",borderRadius:14,border:`1px solid ${theme.border}`,boxShadow:"0 2px 8px rgba(196,30,58,0.06)",overflow:"hidden"}}>
        {(tab==="all" ? allLoading : pendLoading) ? (
          <div style={{padding:24}}>{[...Array(5)].map((_,i)=><div key={i} className="skeleton" style={{height:52,marginBottom:8}}/>)}</div>
        ) : displayUsers.length===0 ? (
          <EmptyState message={tab==="pending"?"No accounts pending approval":"No users found"}/>
        ) : (
          <table>
            <thead><tr style={{background:"#FEF2F2"}}>
            ...
            <tbody>
              {displayUsers.map(u=>(
                <tr key={u.id}>
                  <td>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${theme.red},${theme.turquoise})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"white",flexShrink:0}}>
                        {(u.full_name||"U").charAt(0)}
                      </div>
                      <div>
                        <p style={{fontWeight:600,fontSize:13}}>{u.full_name}</p>
                        <p style={{fontSize:11,color:theme.textMuted}}>@{u.username} · {u.email||"no email"}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    <Badge color={u.is_approved?"green":"amber"}>
                      {u.is_approved?"Active":"Pending"}
                    </Badge>
                  </td>
                  <td style={{fontSize:12,color:theme.textMuted}}>
      ...
      {modal==="create"&&(
        <Modal title="Create User Account" onClose={()=>setModal(null)} large>
          <div style={{padding:12,background:"#F0FDFF",borderRadius:8,marginBottom:16,fontSize:12,color:"#0E7490",borderLeft:`3px solid ${theme.turquoise}`}}>
            Accounts created here are <strong>immediately active</strong> — no approval needed.
          </div>

          <div className="form-row">
            <div className="form-group">
              <label>Role *</label>
              <select className="form-input" value={form.role||"teacher"} onChange={f("role")}>
                {ROLE_OPTIONS.map(r=><option key={r.value} value={r.value}>{r.label}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>Campus *</label>
              <select className="form-input" value={form.campus||"swla"} onChange={f("campus")}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="e.g. Mrs. T. Moyo" value={form.fullName||""} onChange={f("fullName")}/>
          </div>
          <div className="form-group">
            <label>Email Address</label>
            <input type="email" className="form-input" placeholder="e.g. tmoyo@stillwaters.co.zw" value={form.email||""} onChange={f("email")}/>
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>Username * <span style={{fontSize:10,color:theme.textMuted}}>(used to log in)</span></label>
              <input className="form-input" placeholder="e.g. tmoyo" value={form.username||""} onChange={f("username")}/>
            </div>
            <div className="form-group">
              <label>Password * <span style={{fontSize:10,color:theme.textMuted}}>(min 6 characters)</span></label>
              <input type="password" className="form-input" placeholder="Set initial password" value={form.password||""} onChange={f("password")}/>
            </div>
          </div>

          {/* Parent-specific: link to student */}
          {form.role==="parent"&&(
            <div style={{marginTop:4,padding:16,borderRadius:10,background:"#F0FDFF",border:`1px solid ${theme.turquoise}`}}>
              <p style={{fontSize:13,fontWeight:700,color:theme.turquoise,marginBottom:10}}>
                <Icon d={icons.students} size={14} style={{marginRight:6}}/>
                Link to Learner (optional but recommended)
              </p>
              <p style={{fontSize:12,color:theme.textMuted,marginBottom:10}}>Search for the learner to link this parent account to. You can also link them later from the student's profile.</p>
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                <input className="form-input" style={{flex:1}} placeholder="Search by learner surname or ID..."
                  value={linkStudentId} onChange={e=>{setLinkStudentId(e.target.value);setLinkError("");}}
                  onKeyDown={e=>e.key==="Enter"&&searchStudentsForLink()}/>
                <Btn size="sm" variant="tq" onClick={searchStudentsForLink} loading={linkLoading} icon="search">Search</Btn>
              </div>
              {linkError&&(
                <div style={{padding:"8px 12px",background:"#FEF2F2",borderRadius:8,fontSize:12,color:theme.danger,marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
                  <Icon d={icons.alert} size={14} color={theme.danger}/>
                  {linkError}
                </div>
              )}
              {linkSearchResults.length>0&&(
                <div style={{border:`1px solid ${theme.border}`,borderRadius:8,overflow:"hidden",marginBottom:8}}>
                  {linkSearchResults.map(s=>(
                    <div key={s.id} onClick={()=>{setForm(p=>({...p,linkedStudentId:s.id,linkedStudentName:`${s.last_name}, ${s.first_name}`}));setLinkSearchResults([]);setLinkStudentId(`${s.last_name}, ${s.first_name}`);}}
                      style={{padding:"10px 14px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${theme.border}`,background:form.linkedStudentId===s.id?"#F0FDFF":"white"}}
                      onMouseOver={e=>e.currentTarget.style.background="#F8FAFC"}
                      onMouseOut={e=>e.currentTarget.style.background=form.linkedStudentId===s.id?"#F0FDFF":"white"}>
                      <div>
                        <p style={{fontWeight:600,fontSize:13}}>{s.last_name}, {s.first_name}</p>
                        <p style={{fontSize:11,color:theme.textMuted}}>{FORM_LABEL[s.grade]||s.grade} · ID: {s.student_id}</p>
                      </div>
                      {form.linkedStudentId===s.id&&<Icon d={icons.check} size={16} color={theme.turquoise}/>}
                    </div>
                  ))}
                </div>
              )}
              {form.linkedStudentName&&(
                <div style={{padding:"8px 12px",background:"#D1FAE5",borderRadius:8,fontSize:12,fontWeight:600,color:"#065F46",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ✓ Linked to: {form.linkedStudentName}
                  <button onClick={()=>setForm(p=>({...p,linkedStudentId:null,linkedStudentName:null}))} style={{background:"none",border:"none",cursor:"pointer",fontSize:11,color:"#065F46",fontWeight:600}}>Remove ×</button>
                </div>
              )}
            </div>
          )}
